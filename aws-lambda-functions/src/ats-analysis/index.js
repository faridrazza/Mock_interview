...
exports.handler = async (event) => {
  console.log('ATS Analysis Lambda started');
  ...
  try {
    ...
    if (!resumeContent) {
      return createErrorResponse('Missing resume content', 400);
    }
...
function createContentHash(resumeContent, jobDescription = '', templateId = 'raw') {
...
function convertResumeToText(resumeContent) {
  const sections = [];
  ...
  if (resumeContent.skills && resumeContent.skills.length > 0) {
    sections.push("\nSKILLS:");
    sections.push(resumeContent.skills.filter((s) => s.trim()).join(", "));
  }
