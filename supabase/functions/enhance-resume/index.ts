...
// Validate experience entries to ensure they match expected structure
function validateExperienceEntries(experiences: any[]): ExperienceEntry[] {
...
// Validate project entries to ensure they match expected structure
function validateProjectEntries(projects: any[]): ProjectEntry[] {
...
Current summary to enhance: "${content}"`;
      ...
      case 'all':
        ...
        if (improvement_suggestions.length === 0 && missing_keywords.length === 0) {
          ...
          return new Response(
            JSON.stringify({ error: 'No improvement suggestions or missing keywords provided' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
...
Experience entries to enhance:`;
        ...
        if (!Array.isArray(resumeContent)) {
          ...
          return new Response(
            JSON.stringify({ error: 'Experience data must be an array' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
...
Projects to enhance:`;
        ...
        if (!Array.isArray(resumeContent)) {
          ...
          return new Response(
            JSON.stringify({ error: 'Projects data must be an array' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
...
Current skills to enhance:`;
        ...
        if (!Array.isArray(resumeContent)) {
          ...
          return new Response(
            JSON.stringify({ error: 'Skills data must be an array' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
      ...
      default:
        return new Response(
          JSON.stringify({ error: 'Invalid section type' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
    }
...
Your goal is to make the resume content more compelling and effective while maintaining its authenticity and professional credibility.`
    ...
    switch (sectionType) {
      ...
      case 'projects':
        ...
        break;
      case 'skills':
        processedContent = enhancedContent.split(',').map((skill: string) => skill.trim());
        break;
      default:
        processedContent = enhancedContent.trim();
    }
    ...
    return new Response(
      JSON.stringify({ enhanced: processedContent }),
